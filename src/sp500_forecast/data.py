...
def download_sp500(config: AppConfig) -> Path:
    ...
    out_path = resolve_path(config.data.raw_output)
    if out_path is None:
...
def load_price_data(config: AppConfig) -> pd.DataFrame:
    ...
    if csv_path is None:
        csv_path = resolve_path(config.data.raw_output)
...
def standardize_price_frame(frame: pd.DataFrame, *, date_column: str = "Date") -> pd.DataFrame:
    """Normalize common Yahoo/Investing CSV column variants to Date, High, Low."""
    if frame.empty:
