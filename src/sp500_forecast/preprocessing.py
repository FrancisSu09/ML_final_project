...
class StandardScaler2D:
    ...
    def fit(self, values: np.ndarray) -> "StandardScaler2D":
        values = np.asarray(values, dtype=float)
        if values.ndim != 2:
            raise ValueError("StandardScaler2D expects a two-dimensional array")
...
def make_supervised(
    ...
    *,
    ...
    if features is not None:
        ...
        if len(feature_values) != len(series):
            raise ValueError("features must have the same row count as series")

    xs: list[np.ndarray] = []
    ys: list[float] = []
    indices: list[int] = []
    for end in range(window_size, len(series)):
        component_window = series[end - window_size : end, None]
        if feature_values is None or feature_values.shape[1] == 0:
            xs.append(component_window)
        else:
            feature_window = feature_values[end - window_size : end]
            xs.append(np.concatenate([component_window, feature_window], axis=1))
