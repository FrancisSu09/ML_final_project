...
def _run_walk_forward_decomposition(
    *,
    ...
    validation_windows, validation_window_diagnostics = _precompute_walk_forward_windows(
        signal=signal,
    ...
    test_windows, test_window_diagnostics = _precompute_walk_forward_windows(
        signal=signal,
        start=split_index,
        end=len(signal),
        component_names=component_names,
...
def _predict_windows(
    ...
    *,
    ...
    x_scaled = forecaster.scaler.transform(np.asarray(windows, dtype=float)).reshape(
        len(windows),
        windows.shape[1],
        1,
    )
...
def _load_cached_params(path: Path, *, config: AppConfig) -> dict[str, dict[str, int | float]]:
    ...
    for key, value in expected.items():
        if key in payload and payload[key] != value:
            return {}
    if config.decomposition.scope == "train_only_recursive":
        if payload.get("cache_schema") != "fit_only_recursive_v2":
            return {}
    if config.decomposition.scope == "walk_forward":
        if payload.get("cache_schema") != "walk_forward_v2":
            return {}
